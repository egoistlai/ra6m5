...
static void keypad_read(lv_indev_drv_t *indev_drv, lv_indev_data_t *data)
{
    ...
    if (act_key != 0)
    {
        ...
        {
        case 1:
            act_key = LV_KEY_NEXT; // 摇杆: 右 -> 下一个焦点
            break;
        case 2:
            act_key = LV_KEY_PREV; // 摇杆: 左 -> 上一个焦点
            break;
        case 3:
            act_key = LV_KEY_LEFT; // 摇杆: 上 -> 减少/左移 (视逻辑而定，也可设为 PREV)
            break;
        case 4:
            act_key = LV_KEY_RIGHT; // 摇杆: 下 -> 增加/右移 (视逻辑而定，也可设为 NEXT)
            break;
        case 5:
            act_key = LV_KEY_ENTER; // 摇杆: 按下 -> 确认
            break;
        }
        last_key = act_key;
    }
...
{
    /* 1. 先检测是否按下确认键 */
    if (drv_uart_get_pin_state(EC11_PIN_KEY)) // 假设低电平有效，你的驱动返回true表示按下
    {
    ...
    /* 2. 检测摇杆方向 */
    /* 注意：你的 drv_joystick_get_dir() 可能会一直返回方向，需要做防抖或者边缘检测吗？
       LVGL 会定期调用这个函数，如果一直返回 UP，光标会跑得飞快。
       但 LVGL 内部有处理长按逻辑，通常直接返回当前状态即可。
    */
    joy_dir_t dir = drv_joystick_get_dir();
    ...
    case JOY_LEFT:
        return 2; // PREV
    case JOY_UP:
        return 3; // 这里将 上 也映射为 PREV (或者你可以用 LV_KEY_UP)
    case JOY_DOWN:
        return 4; // 这里将 下 也映射为 NEXT (或者你可以用 LV_KEY_DOWN)
    default:
        return 0;
    }
}
