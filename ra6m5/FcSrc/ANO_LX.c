...
_fc_alt_un fc_alt;
_fc_acc_un fc_acc;
_fc_my_un fc_my_data;
_fc_my1_un fc_my1_data;
...
static inline void RC_Data_Task(float dT_s)
{
    ...
    if (rc_in.fail_safe == 0)
    {
        ...
        // 实时XYZ-YAW期望速度(实时控制帧)
        rt_tar.st_data.yaw_dps = control_yaw; // 航向转动角速度，度每秒，逆时针为正
        rt_tar.st_data.vel_x = control_x;     // 头向速度，厘米每秒
        rt_tar.st_data.vel_y = control_y;     // 左向速度，厘米每秒
        rt_tar.st_data.vel_z = control_z;     // 天向速度，厘米每秒
        // ########################################################################
        //=====
...
// 校准ESC成功后，记得关闭此功能，避免出现意外。
    ...
    else
    {
        ...
        if (unlocked)
        {
            for (u8 i = 0; i < 8; i++)
            {
                // if (takeoff_height != 0 && control_z > -3){
                // 	pwm[i] = LIMIT(pwm[i], 450, 1000);
                // }
    ...
    fc_my_data.st_data.x = pwm[0];
    fc_my_data.st_data.y = pwm[1];
    fc_my_data.st_data.z = pwm[2];
    fc_my_data.st_data.yaw = pwm[3];
    // 给底层PWM驱动输出信号
    //这里可以可视化pwm数据
    //DrvMotorPWMSet(pwm);
}
