...
u8 imu_update_cnt = 0;     // IMU数据更新计数
u8 imu_acc_update_cnt = 0; // IMU加速度计数据更新计数
_dt_st dt;
...
void ANO_DT_Init(void)
{
    ...
    dt.fun[0xF1].fre_ms = 50;     // 触发发送的周期50ms
    dt.fun[0xF1].time_cnt_ms = 0; // 设置初始相位，单位1ms
    //
    dt.fun[0xF2].D_Addr = 0xff;
    dt.fun[0xF2].fre_ms = 50;     // 触发发送的周期50ms
...
static void ANO_DT_LX_Data_Receive_Anl(u8 *data, u8 len)
{
    ...
    else if (*(data + 2) == 0X07)
    {
        ...
        imu_update_cnt++;
        // DrvUart2SendBuf((u8 *)&fc_vel, sizeof(fc_vel));
    }
    ...
    else if (*(data + 2) == 0X03)
    {
        for (u8 i = 0; i < 9; i++)
        {
            ...
            // printf("[%d]: 0x%02X ", i, fc_att_qua.byte_data[i]);
        }

        // printf(fc_att_qua.byte_data);
    }
    // 传感器数据
    else if (*(data + 2) == 0X01)
    {
...
static void Add_Send_Data(u8 frame_num, u8 *_cnt, u8 send_buffer[])
{
    ...
    case 0xF1:
    {
        for (u8 i = 0; i < 20; i++)
        {
            send_buffer[(*_cnt)++] = fc_my_data.byte_data[i];
        }
    }
    break;
    case 0xF2:
    {
        for (u8 i = 0; i < 8; i++)
        {
            send_buffer[(*_cnt)++] = fc_my1_data.byte_data[i];
        }
    }
    break;
    case 0x0d: // 电池数据
